...
def history():
    ...
    records = conn.execute(
        "SELECT * FROM predictions WHERE user_id = ? ORDER BY analyzed_at DESC",
        (session["user"]["id"],)
    ).fetchall()
    conn.close()
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            if session.get("user"):
                conn = get_db_connection()
                conn.execute(
                    "INSERT INTO predictions (user_id, filename, prediction) VALUES (?, ?, ?)",
                    (session["user"]["id"], filename, pred_label)
                )
                conn.commit()
                conn.close()

            return render_template("result.html", filename=filename, prediction=pred_label)
